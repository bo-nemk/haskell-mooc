...
capitalizeFirst s = map toUpper (take 1 s) ++ drop 1 s
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (\x -> x <= max) [k ^ p | p <- [0..]]
...
joinToLength n s = filter (\x -> length x == n) [x ++ y | x <- s, y <- s] 
...
--   [] +|+ []            ==> []
(+|+) :: [a] -> [a] -> [a]
a +|+ b = (if length a > 0 then [head a] else []) ++ (if length b > 0 then [head b] else [])
...
multiCompose fs = if length fs > 0 then head fs . multiCompose (drop 1 fs) else id
...
interpreter xs = interpreterHelper xs 0 0
interpreterHelper ("up":rest) x y = interpreterHelper rest x (y + 1)
interpreterHelper ("down":rest) x y = interpreterHelper rest x (y - 1)
interpreterHelper ("left":rest) x y = interpreterHelper rest (x - 1) y
interpreterHelper ("right":rest) x y = interpreterHelper rest (x + 1) y
interpreterHelper ("printX":rest) x y = show x : interpreterHelper rest x y
interpreterHelper ("printY":rest) x y = show y : interpreterHelper rest x y
