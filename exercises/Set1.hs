...
distance x1 y1 x2 y2 = sqrt $ mysum (mypow $ mydif x1 x2) (mypow $ mydif y1 y2)
...
weight0 x = x <= 500 
weight1 :: Int -> Bool
weight1 x = x > 500 && x <= 5000

postagePrice x = if weight0 x then 250 else if weight1 x then 300 + x else 6000
...
ilog3 1 = 1
ilog3 2 = 1
ilog3 n = if div n 3 > 0 then 1 + (ilog3 $ div n 3) else 0
